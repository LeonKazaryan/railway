...
export function TwinAlertBanner({ snapshot }: TwinAlertBannerProps) {
  ...
  const hasFaults =
    snapshot.fault_codes.length > 0 || snapshot.alarm_status > 0;
  const faultLabels = snapshot.fault_codes
    .map((code) => t(`faultCodes.${code}`, { defaultValue: code }))
    .slice(0, 2);
  const count = snapshot.fault_codes.length;
  const alertCountLabel =
    count === 1
      ? t("twin.alertBanner.activeSingle", { count })
      : t("twin.alertBanner.activePlural", { count });
          ...
          <div className="flex items-start gap-2.5">
            ...
            <div className="flex flex-col gap-0.5">
              ...
              {snapshot.fault_codes.length > 2 && (
                ...
                >
                  {t("twin.alertBanner.more", {
                    count: snapshot.fault_codes.length - 2,
                  })}
                </span>
              )}
            </div>
